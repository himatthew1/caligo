...
//   { n: 이름, d: [t1슬롯, t2슬롯, t3슬롯], c: 콤보 의도, k: 견제(어떤 픽에 강한가) }
//   ★ 용병(mercenary)은 FREE_TIER — 티어 구분 없이 어느 슬롯에나 배치 가능(자유 배치가 정체성).
//     그래서 용병 덱은 일부러 용병을 t1/t2 슬롯에 넣어 저티어 슬롯을 강화하는 색다름을 살림.
const AI_DECK_LIST = [
  // ── 순수 왕실(Royal) 5 ──
  ...
  // ── 순수 무팩션(No-faction) 5 ──
  { n: '강철 용병단',   d: ['mercenary', 'armoredWarrior', 'militia'],     c: '용병을 t1 슬롯에 자유배치(저티어를 십자 atk2로 강화)+아이언스킨 방벽+민병 화력', k: '저티어 러시(약한 t1 슬롯도 강력)' },
  ...
  { n: '운명의 도박판', d: ['fortuneTeller', 'mercenary', 'hero'],         c: '용병을 t2 슬롯에 자유배치(십자 atk2) + 흉조 교란 + 영웅 대지분쇄', k: '설치물·밀집 의존 덱(대지분쇄)' },
  // ── 하이브리드 30 (2팩션 이상 혼합) ──
