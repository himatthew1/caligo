...
const CHARACTERS = {
  ...
  2: [
    ...
    { type:'witch', name:'마녀', tier:2, atk:0, icon:'/assets/icons/witch.png', tag:'villain', noAttack:true, desc:'공격 불가',
      skills:[
    ...
    { type:'dryad', name:'드라이어드', tier:2, atk:2, icon:'🌳', tag:'spirit', desc:'가로 3칸', noRemains:true,
      skills:[], passives:['growth'] },
    { type:'ironman', name:'철인', tier:2, atk:0, icon:'🏋️', tag:null, desc:'제자리와 상단',
      skills:[], passives:['might'] },
    { type:'heretic', name:'이단자', tier:2, atk:1, icon:'🕯️', tag:'villain', desc:'세로 3칸',
    ...
    { type:'courtier', name:'궁정대신', tier:2, atk:1, icon:'🎩', tag:'royal', desc:'제자리 포함 U자',
      skills:[], passives:['suppression'] },
    { type:'golem', name:'골렘', tier:2, atk:3, icon:'🗿', tag:'spirit', noRemains:true, desc:'세로 3칸',
      skills:[], passives:['oldHeart'] },
    { type:'executioner', name:'처형인', tier:2, atk:1, icon:'🪓', tag:'royal', desc:'제자리와 아래 대각 2칸',
  ...
  3: [
    ...
    { type:'undead', name:'언데드', tier:3, atk:1, icon:'🧟', tag:'villain', noRemains:true, desc:'양옆만 공격',
      passives:['rottenSoul'], skills:[] },
    { type:'demonKing', name:'마왕 칼리고', tier:3, atk:0, icon:'👹', tag:'villain', desc:'제자리 중심 V자 6칸',
    ...
    { type:'mercenary', name:'용병', tier:3, atk:2, icon:'🗡️', tag:null, desc:'십자', skills:[] },
...
function aiPlacementCellScore(piece, c, r, ctx) {
  ...
  if (piece.type !== 'bodyguard') {
    // ★ 사용자 피드백: AI가 '자기 유닛끼리' 항상 뭉침 → 분산 회피 대상을 팀원뿐 아니라 '자기 포함'
    //   모든 아군(friendlyIdxs)으로 확대(근본: 팀전은 teammateIdxs 에 자기가 빠져 자기 유닛 분산이 전무했음).
    //   aoeMul 하한(0.7)으로 적 광역이 없어도 무의미한 밀집은 억제 — 단 지휘관/호위무사 시너지(+10)는
    //   이 패널티를 이기므로 '지휘관 픽 시 뭉침'은 그대로 유지(사용자: 지휘관이면 뭉치는 게 답).
    for (const tIdx of friendlyIdxs) {
      const tp = room.players[tIdx]; if (!tp) continue;
      for (const tpc of (tp.pieces || [])) {
        if (!tpc.alive || tpc.col < 0 || tpc === piece) continue;
        const md = Math.abs(tpc.col - c) + Math.abs(tpc.row - r);
        if (md === 1) score -= 6 * Math.max(0.7, et.aoeMul);
...
// ★ AI 후보덱 50종은 ai-decks.js 로 분리 — 목록 편집은 그 파일에서.
const { AI_DECK_LIST } = require('./ai-decks');
