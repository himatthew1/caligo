...
const CHARACTERS = {
  1: [
    ...
    { type:'catapult', name:'투석기', tier:1, atk:2, icon:'🪨', tag:'royal', noRemains:true, desc:'단일 저격(원하는 1칸) · 일반 이동 불가, 구동으로만 이동',
    ...
    // ── ★ Phase 3 신규(무스킬·무패시브) ──
    { type:'wanderer', name:'방랑자', tier:1, atk:1, icon:'🧭', tag:null, desc:'도약 — 나이트 8칸', skills:[] },
    { type:'hookKiller', name:'갈고리 살인마', tier:1, atk:1, icon:'🪝', tag:'villain', desc:'위치한 곳 가로줄 전체 공격', skills:[] },
    { type:'gravekeeper', name:'묘지기', tier:1, atk:1, icon:'⚰️', tag:'villain', desc:'제자리 제외 십자 · 담력: 유해당 +0.5, 유해 칸 이동 가능 · 도굴로 유해 제거+SP',
      skills:[{id:'exhume', name:'도굴', cost:0, replacesAction:false, desc:'공격범위 내 유해 하나를 제거하고 인스턴트 SP 2 획득'}], passives:['valor'] },
    { type:'fairy', name:'요정', tier:1, atk:0.5, icon:'🧚', tag:'spirit', desc:'H자(좌우 세로3+제자리)',
    ...
    { type:'poisoner', name:'독살꾼', tier:1, atk:1, icon:'🧪', tag:'villain', desc:'제자리와 X자 대각선 · 독니: 공격 대상 중독',
      skills:[{id:'venomCloud', name:'맹독 구름', cost:2, replacesAction:true, desc:'공격 범위 내 모든 적 중독'}], passives:['venomFang'] },
    { type:'thief', name:'도적', tier:1, atk:1, icon:'🦹', tag:null, desc:'좌우 세로2씩(자기줄+상단)',
      skills:[{id:'steal', name:'강탈', cost:0, replacesAction:true, desc:'상대보다 SP가 적으면 상대 공유 SP 1 탈취'}] },
    { type:'fortuneTeller', name:'포춘텔러', tier:1, atk:0.5, icon:'🔮', tag:null, desc:'십자(제자리+상하좌우)',
      skills:[
  ...
  2: [
    ...
    { type:'griffin', name:'그리폰', tier:2, atk:2, icon:'🦅', tag:'spirit', desc:'세로 3칸 · 격노: 피해 받으면 활성', noRemains:true,
      skills:[{id:'rage', name:'격노', cost:2, replacesAction:false, oncePerTurn:true, desc:'피해 받으면 활성 · 적 1명에게 1 피해'}], passives:['rage'] },
    { type:'dryad', name:'드라이어드', tier:2, atk:2, icon:'🌳', tag:'spirit', desc:'가로 3칸 · 생장: 피해 받을 때마다 사거리 +1', noRemains:true,
      skills:[], passives:['growth'] },
    { type:'ironman', name:'철인', tier:2, atk:0, icon:'🏋️', tag:null, desc:'제자리와 상단 · 괴력: 공격력 = 남은 체력',
      skills:[], passives:['might'] },
    { type:'heretic', name:'이단자', tier:2, atk:1, icon:'🕯️', tag:'villain', desc:'세로 3칸 · 현혹: 공격 대상 이교단화(전원 이교단 시 승리)',
      skills:[], passives:['enthrall'] },
    { type:'necromancer', name:'악령술사', tier:2, atk:1, icon:'🪦', tag:'villain', desc:'제자리+위 대각2 · 강령술로 유해를 악령으로 부활 · 조종으로 악령 일제 공격',
      skills:[
    ...
    { type:'storyteller', name:'이야기꾼', tier:2, atk:0.5, icon:'📖', tag:null, desc:'자기줄+상단 2×3 · 선동으로 배신 유발',
      skills:[{id:'incite', name:'선동', cost:3, replacesAction:false, oncePerTurn:true, desc:'적 1명 배신 상태(전원 배신 시 분란 승리) · 이야기꾼 피격 시 해제'}] },
    { type:'courtier', name:'궁정대신', tier:2, atk:1, icon:'🎩', tag:'royal', desc:'제자리 포함 U · 탄압: 비왕실 스킬 SP +1',
      skills:[], passives:['suppression'] },
    { type:'golem', name:'골렘', tier:2, atk:3, icon:'🗿', tag:'spirit', noRemains:true, desc:'세로 3칸 · 낡은 심장: 지급 체력에서 2 낮게 시작',
      skills:[], passives:['oldHeart'] },
    { type:'executioner', name:'처형인', tier:2, atk:1, icon:'🪓', tag:'royal', desc:'제자리+아래 대각2 · 참수: 공격받은 적 처형(패시브 무력화, 언데드는 즉시 파괴)',
  ...
  3: [
    ...
    { type:'undead', name:'언데드', tier:3, atk:1, icon:'🧟', tag:'villain', noRemains:true, desc:'양옆만 공격 · HP0 시작 · 부패한 영혼: 항상 유해로 간주되나 정상 플레이(참수·대지분쇄·보드파괴로만 소멸)',
      passives:['rottenSoul'], skills:[] },
    { type:'demonKing', name:'마왕 칼리고', tier:3, atk:0, icon:'👹', tag:'villain', desc:'제자리 중심 V(6칸) · 어둠장막: 악인 외 전 유닛 공격범위 1칸 봉인',
      skills:[{id:'corrupt', name:'타락', cost:1, replacesAction:true, desc:'보드 위 모든 악인 공격력 +0.5(누적)'}],
      passives:['darkVeil'] },
    // ── ★ Phase 3 신규 ──
    { type:'siegeBreaker', name:'공성파괴자', tier:3, atk:2, icon:'🔨', tag:'royal', desc:'제자리+하단1 · 파괴공작: 지정 칸이 다음 내 턴 시작에 파괴',
      skills:[{id:'demolish', name:'파괴공작', cost:4, replacesAction:false, desc:'한 칸 지정 → 다음 자신의 턴 시작과 동시에 영구 파괴(그 위 유닛 소멸)'}] },
    { type:'mushkin', name:'머쉬킨', tier:3, atk:1, icon:'🍄', tag:'spirit', noRemains:true, desc:'자기줄+하단 2×3 · 포자살포(피격 시 랜덤 2곳 진균화) · 진균칸서 턴 마친 유닛 중독(머쉬킨 면역)',
      passives:['sporeSpread'],
      skills:[{id:'spread', name:'확산', cost:2, replacesAction:false, desc:'모든 진균지대의 인접 칸 중 하나씩 새 진균지대로 확산'}] },
    { type:'oberon', name:'오베론', tier:3, atk:1, icon:'🧝', tag:'spirit', desc:'제자리+하단 대각2 · 요정왕 카운터(정령 피해 시 +1)로 축복/은혜/종언',
      passives:['faeKing'],
    ...
    { type:'militia', name:'민병대장', tier:3, atk:3, icon:'🛡️', tag:null, desc:'가로 3칸', skills:[] },
    { type:'mercenary', name:'용병', tier:3, atk:2, icon:'🗡️', tag:null, desc:'십자 — 자유 티어 배치', skills:[] },
    { type:'hero', name:'영웅', tier:3, atk:3, icon:'🦸', tag:null, desc:'제자리와 하단',
    ...
    { type:'homunculus', name:'호문클루스', tier:3, atk:2, icon:'🧫', tag:null, desc:'좌우 + 하단 대각2',
  ...
  switch (effectiveType) {
    ...
    case 'necromancer': {
      ...
      if (nSkill === 'raise') {
        ...
        wraith.type = 'wraith'; wraith.name = '악령'; wraith.icon = '👻';
