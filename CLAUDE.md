...
- **핵심: 내 말 셀 클릭 → 라디얼 메뉴 등장** (`.radial-btn` = "🏃이동" / "⚔공격", 선택 말 셀은 `.radial-active`).
  ...
  - **공격**: ⚔공격 클릭 → `.cell.attack-range` 하이라이트 → 사거리 내 셀 클릭(적이 숨어있어도 빈칸 공격 가능 = blind). **효과는 사거리 전체 동시 타격**(단일타깃 shadowAssassin/witch만 클릭 칸이 실제 타깃). 캐릭터별 사거리 다름 (예: 창병=세로 일직선 전체가 한 번에 맞음).
...
- **공격 = 사거리 전체 동시 타격이 기본** (❗2026-07 정정, 이전 "단일 타깃" 기술은 오류): 창병=세로열 전체, 파수꾼=주변8칸 등 **사거리 내 모든 칸의 적이 동시에 피격**됨 (server `processAttack` 이 `atkCells` 전 칸·전 적에 `resolveDamage`). **단일 타깃은 극소수 특성** — 현재 `shadowAssassin`(3×3 중 1칸)·`witch`(원거리 1칸)만 tCol/tRow 로 한 칸 선택. → blind 여도 넓게 덮으면 클립 가능성↑ (넓은 사거리 = 장점).
...
- 예(실측 판·추론 *방법* 예시 — ⚠️화약상 범위는 리워크 전 옛값이라 현재와 다름: 지금은 주변8칸 랜덤2): 내 장군(1,1)이 1피해→화약상(ATK1, 세로 ±1·±2) ⇒ 화약상∈{(1,0),(1,2),(1,3)}. 창병(2,4)이 2피해→갑주무사(ATK2, 아래 3칸) ⇒ 갑주무사∈{(1,3),(2,3),(3,3)}. ⇒ 내 장군(+모양)으로 (1,2) 또는 창병(세로열)으로 (2,3) 반격이 유력타.

### 📋 캐릭터 공격범위 표 (server.js `getAttackCells`, piece (c,r) 기준 · bounds 클리핑)
- **T1** archer=대각선 직선(토글 ↘/↙) · spearman=세로열 전체 · cavalry=가로행 전체 · watchman=주변8칸 · twins_elder/scout=가로3(c,c±1) · twins_younger/manhunter=세로3(r,r±1) · messenger=자기+대각4 · **gunpowder=주변 8칸(3×3 자기 제외)이 '범위후보'이나 실제 명중은 그 중 랜덤 2칸(processAttack). UI 도 후보=어두운 주황+랜덤 2칸 점멸, 표시셀(공격자/표식상대/관전자)은 실제 2칸(_effAtkCells)** · herbalist=가로 (c±1,c±2)
...
- **SP 경제** (❗2026-07 정정): 양측 **시작 SP `[1,1]` 보유**(server 513), **후공(2번째)은 인스턴트 SP +1 추가 시작**(선공 이점 보정, server 3584). 이후 **턴 10·20·30·40(turnNumber%10==0, ≤40)** 에 양측 +1(상한 10, 풀 균형, server 5042). instant SP(`instantSp`)는 후공 시작분 + 패시브(마법사 instantMagic 등)로 획득. 스킬 비용은 `sp+instantSp` 합산에서 차감. SP 부족 시 스킬 버튼 비활성. (이전 "시작 SP 없음 / 10·20·40" 기술은 오류였음.)
...
- **SP**: 풀 기반, **시작 `[1,1]` + 후공 인스턴트 +1**, 이후 턴 10·20·30·40 에 +1(상한10). instant SP 별도. (상세는 위 스킬 섹션)
